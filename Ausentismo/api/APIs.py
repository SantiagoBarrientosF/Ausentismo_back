...
def get_data_api(cedula):
    ...
    try:
        ...
        response = requests.get(api_url, headers=headers,timeout=(10,10))
...
def get_data_api_Gestiones(cedula,tipo_gestion):
    print("llamando la api..")
    # URL de la API a la que se realiza la solicitud, basada en el valor de 'cedula'
...
def get_data_api_all(self):
    print("llamando la api..")
    # URL de la API a la que se realiza la solicitud, basada en el valor de 'cedula'
    api_url = f"{settings.EXTERNAL_API_URL_AUSENTISMO_USERS}"
    headers = {
        "accept": "application/json",
        'Connection': 'keep-alive',
    }
